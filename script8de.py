    ...
    for linea in lineas:
        try:
            ...
            if len(data["sentencias_disagree"])<len(data["sentencias_NOT_disagree"]):
                ...
                # print(segmentos_no_negacionistas[data["identifier"]])
                #print(clasificador.predict(segmentos_no_negacionistas[data["identifier"]]))
                resultado = clasificador.predict(segmentos_noticia)
                #for segmento in segmentos_no_negacionistas[data["identifier"]]:
                #    print(clasificador.predict(segmento))
                #print(calcular_etiquetas(resultado[0]))
